...
class FileProviderExtensionTest {
	...
	public void file_csv() throws HopperException, RepositoryException {
		...
		assertEquals("testfile.csv (text/csv;charset=utf-8):\n" + "\"my,csv\",one\n", runHandler.getLastFile());
	}
	...
	public void file_jsonObject() throws HopperException, RepositoryException {
		...
		assertEquals("my-test-json.json (application/json):\n" + "{\"myProp\":\"propVal\"}", runHandler.getLastFile());
	}
	...
	public void file_jsonArray() throws HopperException, RepositoryException {
		...
		assertEquals("arr.json (application/json):\n" + "[\"arr\",[\"speak like a\",\"pirate\"]]", runHandler.getLastFile());
	}
			...
			.build(
				new Script(
					...
					new RunScript.Config()
						.withExtension("jexl")
						.withCode("txt.append('## Usage');\ntxt.println();\ntxt.append('Use it however you want.')")
				)
			)
			.run(context.resourceResolver().adaptTo(Session.class), true);

		assertEquals("README.txt (text/plain;charset=utf-8):\n" + "## Usage\n" + "Use it however you want.", runHandler.getLastFile());
	}
		...
		runnerService
			...
			.build(
				new Script(
					...
					new RunScript.Config()
						.withExtension("jexl")
						.withCode("txt.mimeType = 'text/javascript';\ntxt.extension = 'js';\ntxt.append('\"use strict\"')")
				)
			)
			.run(context.resourceResolver().adaptTo(Session.class), true);

		assertEquals("main.js (text/javascript):\n" + "\"use strict\"", runHandler.getLastFile());
	}
}
